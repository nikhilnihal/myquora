...
if($number==1)           //agar entry hai to update
{
        ...
        $sql ="UPDATE answer_table SET a_downvote=a_downvote+1  where ans_id=$aya";
        mysql_query($sql);
        $sql ="UPDATE $uid SET comment_down=1   where comment_id=$aya";
        mysql_query($sql);
        }


        else if($d==0 && $u==1) // downvote 1 tha
         {
        $sql ="UPDATE answer_table SET a_upvote=a_upvote-1 , a_downvote=a_downvote+1 where ans_id=$aya";
        mysql_query($sql);
        $sql ="UPDATE $uid SET comment_up=0 , comment_down=1  where comment_id=$aya";
        mysql_query($sql);
